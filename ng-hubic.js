...
angular.module('ngHubic', []);

angular.module('ngHubic').provider('Ovh', function() {
    'use strict';

    var baseUrl = 'https://api.hubic.com/1.0';
    ...
    this.$get = ['$http', '$q', '$cacheFactory', '$window', function($http, $q, $cacheFactory, $window) {
        ...
        function login(urlToRedirect) {
            ...
            // Set redirect URI here
            // Example:
            // var redirect_uri = window.encodeURIComponent('http://localhost:3000/#/hubic');
            var redirect_uri = window.encodeURIComponent('');

            var new_url = baseUrl.slice(0, 21) +
                '/oauth/auth/?' +
                '&response_type=token' +
                '&scope=usage.r%2Caccount.r%2CgetAllLinks.r%2Ccredentials.r%2CsponsorCode.r%2Cactivate.w%2Csponsored.r%2Clinks.drw' +
                '&client_id=' + keys.ak +
        ...
        function getModels(schemaPath, name) {
            return getSchema(schemaPath).then(function(schema) {
                ...
                return schema.models[name] ? schema.models[name] : $q.reject({
                    data: {
                        errorCode: 'NOT_FOUND',
