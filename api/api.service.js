angular.module('routerApp').service('Api', function($q, $http, Ovh) {
    'use strict';
    ...
    this.getRootApis = function() {
        return Ovh.getSchema('/?null').then(function(response) {
            ...
            response.apis = _.filter(response.apis, function(api) {
                return hiddenApis.indexOf(api.path) === -1;
            });
        ...
        _.forEach(parameters, function(param) {
            ...
            if (param.isModel) {
                ...
                if (param.isEnum) {
                    ...
                    var modelProperties = _.pick(api.models[param.dataType].properties, function(val) {
                        return !val.readOnly;
                    });
    ...
    this.getSubApis = function(path) {
        return Ovh.getSchema(path).then(function(subApi) {
            ...
            if (subApi.apis && subApi.apis.length) {
                ...
                subApi.apis = _.filter(subApi.apis, function(api) {
                    return hiddenApis.indexOf(api.path) === -1;
                });
